...
@Component({
  selector: 'app-photo-editor-atractie-turistica',
  templateUrl: './photo-editor.component.html',
  styleUrls: ['./photo-editor.component.css']
})
export class PhotoEditorComponentAtractieTuristica implements OnInit {
  @Input() object: AtractieTuristica | undefined;
  ...
  setMainPhoto(photo: Photo) {
    this.tripPlanningService.setMainPhoto(photo.id,'atractiiTuristice').subscribe({
      next: () => {
  ...
  deletePhoto(photoId: number) {
    this.tripPlanningService.deletePhoto(photoId,'atractiiTuristice').subscribe({
      next: _ => {
        if (this.object) {
  ...
  initializeUploader() {
    this.uploader = new FileUploader({
      url: this.baseUrl + 'atractiiTuristice/add-photo/'+this.object?.id,
