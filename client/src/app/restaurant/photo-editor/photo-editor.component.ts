...
@Component({
  selector: 'app-photo-editor-restaurant',
  templateUrl: './photo-editor.component.html',
  styleUrls: ['./photo-editor.component.css']
})
export class PhotoEditorComponentRestaurant implements OnInit {
  @Input() object: Restaurant | undefined;
  ...
  setMainPhoto(photo: Photo) {
    this.tripPlanningService.setMainPhoto(photo.id,'restaurante').subscribe({
      next: () => {
  ...
  deletePhoto(photoId: number) {
    this.tripPlanningService.deletePhoto(photoId,'restaurante').subscribe({
      next: _ => {
        if (this.object) {
  ...
  initializeUploader() {
    this.uploader = new FileUploader({
      url: this.baseUrl + 'restaurante/add-photo/'+this.object?.id,
